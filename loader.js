...
script.innerHTML = `javascript:(function() {
	...
	function Blooket() {
		alert("Note that if you didn't run this script on a blooket game, the UI won't show up.");
	...
	function enableEditing() {
		console.log(document.body.contentEditable="true"==document.body.contentEditable?"false":"true");
	}
	 
	function disableTabClose() {
		(function () {window.onbeforeunload = function() { return 1; };})()

	}
	
	function historyFlooder() {
		(function(){var num=prompt("How many Times Do You Want This Page To Show Up In your History?");done=false;x=window.location.href;for(vari=1;i<=num;i++){history.pushState(0,0,i==num?x:i.toString());if(i==num){done=true}}if(done===true){alert("History Flooding Successful!\n"+window.location.href+"\nNow Appears In Your History "+num+(num==1?"time.":" Times.\nMade by: mortal#9121"))}})();
	}

	
	function tabDisguise() {
		function gcloak() { var link = document.querySelector("link[rel*='icon']") || document.createElement('link');link.type = 'image/x-icon';link.rel = 'shortcut icon';link.href = 'https://www.pngall.com/wp-content/uploads/9/Google-Drive-Logo-Transparent-180x180.png';document.title = 'My Drive - Google Drive';console.log(document.title);document.getElementsByTagName('head')[0].appendChild(link) };gcloak();setInterval(gcloak, 1000);
	...
	function createPage(title) {
	  ...
	  if (title === "Javascript") {
		  var button = createButton("Enable Editing", enableEditing);
		  var button = createButton("Disable Securly Tab Closing (only for this tab)", disableTabClose);
		  var button = createButton("History Flooder", historyFlooder);
		  var button = createButton("Tab Disguise", tabDisguise);
		  
	  }
	  if (title === "Proxies") {
	  		
		  createText('Holy Unblocker');
	  	  var button = createButton("Link 1", holyubone);
		  var button = createButton("Link 2", holyubtwo);
		  
		  createText('Nebula');
		  var button = createButton("Link 1", nebulaone);
		  var button = createButton("Link 2", nebulatwo);
		  var button = createButton("Link 3", nebulathree);


	  }
	  if (title === "Cheats") {
		  var button = createButton("Blooket UI", Blooket);
	  }
